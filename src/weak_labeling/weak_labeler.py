...
def generate_weak_labels(input_dir: str, output_json: str, model_id: str) -> None:
    ...
    if not image_files:
        logger.info("No JPG or PNG images found in the specified directory.")
        return

    logger.info("Found %d image(s). Starting inference with model '%s' ...", len(image_files), model_id)

    # Wrap the image_files list with tqdm for a progress bar
