...
2. **Image Format:** The script currently looks specifically for `.jpg`, `.jpeg`, or `.JPG` files in the target directory. Ensure your input data matches this format.
3. **Dependencies:** Requires the `inference_sdk` and `python-dotenv` packages.

## How It Works
1. **Initialization:** The script imports configuration variables from `src.config` and initializes the inference client via `src.roboflow_client`.
2. **Scanning:** It uses `glob` to locate all JPG images in the provided input directory.
3. **Inference Loop:** For each image, it queries the Roboflow model.
...
```bash
python scripts/weak_labeler.py --input "path/to/images" --output "path/to/save/annotations.json" --model "model-id/version"
...
| --- | --- | --- | --- | --- |
| `--input` | `-i` | Yes | None | Path to the directory containing the JPG images to be labeled. |
...
```bash
python -m src/weak_labeler.py -i "data/processed/batch_1" -o "data/datasets/weak_labels_batch_1.json"

```

## Error Handling

If an image is corrupted or the API connection drops, the script catches the exception, prints an error to the console, and safely continues to the next image. This ensures a single failure does not crash a labeling job that might contain thousands of files.
